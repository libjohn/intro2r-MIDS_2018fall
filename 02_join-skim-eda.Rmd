...
# IGNORE THIS
# This chucnk, when it ran, transformed the initial 538 data and then wrote it to the data directory.
...
library(here)
write_csv(favorability_popularity_rating, 
          here("data", "538_favorability_popularity.csv"))
```
...
There are different types of joins (e.g. inner, left, right, full, etc.).  See the `dplyr::join` [documentation](http://dplyr.tidyverse.org/reference/join.html) for a more complete explanation. Also note that not all the data joined as might be expected.  Joining on alphanumeric keys can be problematic.  There are several issues tha complicate this type of join, including diacritical characters, case sensitivity, and spaces. Ideally, match on consistent numeric keys.
