...
In the code chunk, below, let's transform the data in two different ways.  Assign the answers to unique objects names:  `answer` and `answer2`.  The difference in the two answers is that one response is a Tibble of 1 row and 4 columns, the other is a data frame of 3 rows and 4 columns.  In the next section we will report those answers dynamically.
